...
// Look Up Tables for partial refresh (which I am not using) yanked
// from the Python example. Without sending these, I could not get images to set.
var LUT_VCOM_7IN5_V2 = [...]byte{
...
func (e *Epd) Init() {
	...
	e.sendCommand(LUT_FOR_VCOM)
	for i := 1; i < 42; i++ {
		e.sendData(LUT_VCOM_7IN5_V2[i])
	}

	e.sendCommand(LUT_BLUE)
	for i := 1; i < 42; i++ {
		// @TODO: Yes, this is mismatched. Python example used 0x21 (LUT_BLUE) and
		// the array it called LUT_WW_7IN5_V2 in its second LUT init batch.
		e.sendData(LUT_WW_7IN5_V2[i])
	}

	e.sendCommand(LUT_WHITE)
	for i := 1; i < 42; i++ {
		// @TODO: Yes, this is mismatched, same as above.
		e.sendData(LUT_BW_7IN5_V2[i])
	}

	e.sendCommand(LUT_GRAY_1)
	for i := 1; i < 42; i++ {
		// @TODO: Yes, this is mismatched, same as above.
		e.sendData(LUT_WB_7IN5_V2[i])
	}

	e.sendCommand(LUT_GRAY_2)
	for i := 1; i < 42; i++ {
		// @TODO: Yes, this is mismatched, same as above.
		e.sendData(LUT_BB_7IN5_V2[i])
	}
	e.waitUntilIdle()
	// END LUT TABLE WORK.
